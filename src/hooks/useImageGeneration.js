...
export const useImageGeneration = () => {
  ...
  const generatePrompts = async (initialPrompt, count) => {
    ...
    try {
      ...
      console.log(`Generating ${validatedCount} prompts for: "${initialPrompt}"`)
      ...
      if (data.prompts) {
        if (Array.isArray(data.prompts)) {
          ...
          prompts = data.prompts.filter(prompt => 
            typeof prompt === 'string' && prompt.length >= 10
          )
            ...
            if (jsonMatch) {
              ...
              if (Array.isArray(parsedPrompts)) {
                prompts = parsedPrompts
                  .map(prompt => typeof prompt === 'string' ? prompt.trim() : String(prompt).trim())
      ...
      console.log(`Successfully parsed ${prompts.length} prompts:`, prompts.map(p => p.substring(0, 50) + '...'))
      
      if (prompts.length === 0) {
        throw new Error('No valid prompts were generated. Please try again with a different initial prompt.')
      }
  ...
  const generateImages = async (photoIds, prompts) => {
    ...
    try {
      ...
      console.log(`Generating images for ${photoIds.length} photos with ${prompts.length} prompts`)
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error(`Image generation timeout after ${timeoutDuration/1000}s`)), timeoutDuration)
      )
